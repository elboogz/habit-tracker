...
/**
 * Backdates `habit.createdAt` and, if needed, `habitId`'s active schedule period (bumping
 * `updatedAt` to `now` alongside either) so every date in `simulatedDates` is a genuine Scheduled
 * Opportunity for it under its currently active schedule content -- see lib/domain/dev-simulation.ts
 * for why the debug tools need both. The one shared place all five debug reducer cases go through
 * for this, so schedule-period backdating can never land in some of them but not others. Returns
 * `habit`/`schedulePeriods` unchanged wherever nothing needs backdating (a daily/unpaused habit
 * whose createdAt already covers the window -- the common case).
 */
function withSimulatedHistory(
  habit: Habit,
  schedulePeriods: HabitSchedulePeriod[],
  simulatedDates: string[],
  now: string,
): { habit: Habit; schedulePeriods: HabitSchedulePeriod[] } {
  const createdAt = backdatedCreatedAt(habit.createdAt, simulatedDates);
  const updatedHabit = createdAt === habit.createdAt ? habit : { ...habit, createdAt, updatedAt: now };
  const updatedPeriods = backdatedSchedulePeriod(habit.id, schedulePeriods, dayKey(), simulatedDates);
  return { habit: updatedHabit, schedulePeriods: updatedPeriods };
}
...
function reducer(state: HabitState, action: Action): HabitState {
  switch (action.type) {
    ...
    case 'debugBackfillLogs': {
      ...
      // action.dates is generated by the caller (Settings' handleSimulateStreak, via
      // widenedForDevSimulation + recentScheduledPatternDates) -- this case only needs to backdate
      // to match what was already generated, not generate anything itself.
      const { habit: updatedHabit, schedulePeriods } = withSimulatedHistory(habit, state.schedulePeriods, action.dates, now);
      const habits = state.habits.map((h) => (h.id === action.habitId ? updatedHabit : h));
      return { ...state, habits, logs: [...filteredLogs, ...newLogs], schedulePeriods };
    }
    case 'debugAdvanceChallenge': {
      ...
      const now = new Date().toISOString();
      let logs = state.logs;
      let schedulePeriods = state.schedulePeriods;
      const updatedHabitsById = new Map<string, Habit>();
      for (const habit of challengeHabits) {
        const amount = habit.type === 'count' ? (habit.targetCount ?? 1) : 1;
        logs = logs.filter((log) => !(log.habitId === habit.id && pastDates.includes(log.date)));
        // Widened for generation only (never persisted as-is) so a habit whose non-daily schedule
        // was only just set still generates against its real weekday pattern instead of falling
        // back to daily for dates before that schedule's effectiveFrom -- see
        // lib/domain/dev-simulation.ts's header comment for why this has to happen before
        // scheduledOpportunitiesInWindow runs, not after.
        const generationPeriods = widenedForDevSimulation(habit.id, schedulePeriods, today);
        ...
        const scheduledDates = scheduledOpportunitiesInWindow(habit, generationPeriods, challenge.durationDays - 1, addDays(today, -1));
        const newLogs = simulatedLogsFor(habit.id, scheduledDates.map((date) => ({ date, completed: true })), amount, now);
        logs = [...logs, ...newLogs];
        const result = withSimulatedHistory(habit, schedulePeriods, pastDates, now);
        updatedHabitsById.set(habit.id, result.habit);
        schedulePeriods = result.schedulePeriods;
      }
      const habits = state.habits.map((h) => updatedHabitsById.get(h.id) ?? h);
    ...
    case 'debugCompleteChallenge': {
      ...
      for (const habit of challengeHabits) {
        ...
        // Widened for generation only -- see debugAdvanceChallenge's identical comment above.
        const generationPeriods = widenedForDevSimulation(habit.id, schedulePeriods, today);
        // Filtered per habit through scheduledOpportunitiesInWindow -- see debugAdvanceChallenge's
        // identical comment above.
        const scheduledDates = scheduledOpportunitiesInWindow(habit, generationPeriods, challenge.durationDays, today);
        const newLogs = simulatedLogsFor(habit.id, scheduledDates.map((date) => ({ date, completed: true })), amount, now);
        logs = [...logs, ...newLogs];
        const result = withSimulatedHistory(habit, schedulePeriods, allDates, now);
        updatedHabitsById.set(habit.id, result.habit);
        schedulePeriods = result.schedulePeriods;
      }
      const habits = state.habits.map((h) => updatedHabitsById.get(h.id) ?? h);
    ...
    case 'debugFillHistory': {
      ...
      state.habits.forEach((habit, index) => {
        ...
        const scheduledDates = scheduledOpportunitiesInWindow(habit, generationPeriods, 30, today);
        const newLogs = simulatedLogsFor(habit.id, scheduledDates.map((date) => ({ date, completed: true })), amount, now);
        logs = [...logs, ...newLogs];
        const result = withSimulatedHistory(habit, schedulePeriods, dates, now);
        updatedHabitsById.set(habit.id, result.habit);
        schedulePeriods = result.schedulePeriods;
      });
      const habits = state.habits.map((h) => updatedHabitsById.get(h.id) ?? h);
      return { ...state, habits, logs, schedulePeriods };
    }
    case 'debugSimulateScenario': {
      ...
      const today = dayKey();
      // Widened for generation only -- see debugAdvanceChallenge's identical comment above.
      const generationPeriods = widenedForDevSimulation(action.habitId, state.schedulePeriods, today);
      const pattern = scenarioPattern(action.scenario, action.habitId, generationPeriods, today);
      ...
      const { habit: updatedHabit, schedulePeriods } = withSimulatedHistory(habit, state.schedulePeriods, windowDates, now);
      const habits = state.habits.map((h) => (h.id === action.habitId ? updatedHabit : h));
      return { ...state, habits, logs: [...filteredLogs, ...newLogs], schedulePeriods };
    }
    case 'resetAllData':
