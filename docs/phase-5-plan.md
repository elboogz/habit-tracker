...
### Cross-kind fallback collision — pending ruling **[New — pending ruling]**

First live acceptance check (2026-09-14) showed Coach (nudge) and Weekly reflection displaying identical fallback text. Confirmed cause: `buildFallbackProvider`'s seed is `userId + dayKey` only, deliberately ignoring `kind` (`coach-fallback.ts`'s own doc comment; `coach-fallback.test.ts` line 43 pins this as intended). Would also affect `monthly` on the same day. Candidate fix — add `kind` to the seed — touches `coach-fallback.ts`, which is `SOURCES`-listed, so it moves the shared `block` hash and requires both functions regenerated, re-pasted, and B6-reverified; not eligible for the §2/§6.8 single-function exception, since it changes cache-content-producing logic. **No ruling made. Pending review with remaining Phase 5 acceptance items.**
...
| **Cross-kind fallback collision, confirmed 2026-09-14 (§6.5, "Cross-kind fallback collision").** Coach and Weekly reflection can show byte-identical fallback text on the same screen, since `buildFallbackProvider`'s seed ignores `kind`. | Ruling pending, not accepted or rejected. |
...
**Status, 2026-09-17.** Items 1-6 satisfied. **Open: item 7** (Acceptance Gate wording, §9, "not yet applied"), **item 8** (documentation reconciliation table above, unaddressed). Also open, outside this list: the §6.3 legacy-symbol cleanup check (not yet performed/reported); the formal Acceptance Gate document (`docs/acceptance-gate-findings.md`, not yet created); the §6.5 cross-kind fallback ruling (pending). Diagnostic removal is **not** on this list — confirmed a Phase 6 precondition only (see "One obligation outlives Phase 5" above).
