...
### Cross-kind fallback collision, amended **[Ruled — amends the seed-resolution ruling above]**
...
| **Cross-kind fallback collision, confirmed 2026-09-14, resolved 2026-09-17 (§6.5, "Cross-kind fallback collision, amended").** Coach and Weekly reflection could show byte-identical fallback text on the same screen, since `buildFallbackProvider`'s seed ignored `kind`. A first fix (hashing `kind` into the seed) was proven insufficient — 52% collision rate over 30,000 sampled pairs — before it shipped. | **Resolved.** Base hash unchanged (`userId + dayKey`, the original formula); `kind` now applies a fixed unique offset (`KIND_OFFSETS`), guaranteeing pairwise-distinct selection for every account/day, proven exhaustively. Message set, FNV-1a, and deterministic selection unchanged. Pending deployment (both functions, per the standing both-together rule). |
...
**Status, 2026-09-17.** Items 1-6 satisfied. **Open: item 7** (Acceptance Gate wording, §9, "not yet applied"), **item 8** (documentation reconciliation table above, unaddressed). Also open, outside this list: the formal Acceptance Gate document (`docs/acceptance-gate-findings.md`, not yet created). **Resolved since the previous status note:** the §6.3 legacy-symbol cleanup check (clean report, removal implemented) and the §6.5 cross-kind fallback ruling (amended, implementation complete) — both pending deployment (both Edge Functions, per the standing both-together rule), not further review. Diagnostic removal is **not** on this list — confirmed a Phase 6 precondition only (see "One obligation outlives Phase 5" above).
