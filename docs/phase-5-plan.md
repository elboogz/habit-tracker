...
**Placement in the generation path, wired in Part 3b.** Runs immediately after `validateCoachOutput` on the same generated text, on the grounded branch only, before anything is cached, displayed, or pushed. Either check failing routes to the same existing rejection path §6.6 already defines — discard the prose, persist the failure sentinel, display/push nothing, never fall through to the deterministic fallback. Not a new consequence type; an additional gate in front of the same `rejected` outcome.

### `combinedValidate`: moved into `lib/domain` after a hand-duplication drift risk was named **[New]**

The first Part 3b pass hand-duplicated the numeric+lexical combinator (`combinedValidate`) identically in both Edge Functions, with nothing enforcing that identity — exactly the §D1 drift class (`docs/phase-5-precondition-review.md`): two producers writing to the same shared `ai_insights` cache with independently maintained validation logic the generated block cannot see. **`combinedValidate` is pure domain composition with no Edge Function dependency at all** (no Deno global, no supabase client, no fetch — it only calls `validateCoachOutput` and `checkProhibitedLexicon`, both already generated), so it moved into `lib/domain/coach-output-check.ts` and into `SOURCES`, making both producers run the identical implementation by construction rather than by a byte-identity test after the fact.

**A real defect was found and fixed in the same pass.** The hand-duplicated version returned `{ valid: false, invalidNumerals: lexical.matchedPhrases }` on a lexical failure — a prohibited phrase like `"getting back on track"` reported in a field named `invalidNumerals`. It compiled, because both are `string[]`, but `buildRejectionDiagnostic` would then report a rejected phrase as though it were a numeral, and the production diagnostic exists specifically to say *why* validation failed. **Fixed by adding an optional `matchedPhrases` field to `ValidationResult`'s `false` branch** (`lib/domain/coach-validation.ts`), rather than a discriminated union carrying a `kind` tag: the additive field requires no change to `validateCoachOutput` itself (which never sets it) and no change to `buildRejectionDiagnostic`/`RejectionDiagnostic` (which keep reading `invalidNumerals` exactly as before, now honestly empty on a purely lexical failure). A discriminated union was considered and rejected as more abstraction than the defect needed: it would have required `validateCoachOutput`'s own return statement and `buildRejectionDiagnostic`'s internals to branch on the new tag, rippling into Step 2's already-settled numeric-only validator for a fix that only concerns the new combinator layered on top of it. Both classes are structurally distinguishable now: `invalidNumerals` stays honestly empty on a lexical-only failure, `matchedPhrases` stays absent on a numeric-only one, and — by construction, since `combinedValidate` checks the numeric validator first and returns immediately on failure — a single result can never carry both populated at once.

**Numeric-first classification, recorded explicitly as intentional, not an unmeasured limitation.** `combinedValidate`'s order is a deliberate rule, not an implementation accident: numeric validation runs first; if it fails, validation stops there and the lexical check is never reached; the lexical check only runs once the numeric check has already passed. Text containing both an invalid numeral and a prohibited phrase is therefore always classified as a numeric rejection only — this is the intended semantics, confirmed directly in `lib/domain/coach-output-check.test.ts` ("both failure classes: not structurally observable by design") and in `scripts/edge-function-wiring.test.ts`'s real-source rejection-diagnostic tests for both Edge Functions, not merely asserted here.

### D1 guarantee across both `ai_insights` producers, stated explicitly **[New]**

Exact byte identity of the whole request/cron paths is not required, and the two legitimately differ (RLS-scoped user client vs. service-role client, one request vs. a per-recipient loop, three kinds vs. one). The invariants that must not drift between `ai-insights` and `send-coaching-push` are narrower, and each is covered a specific way:

| Invariant | Mechanism |
|---|---|
| Same selector (`selectLeadingHabit`) | Generated, `SOURCES`-listed — identical by construction |
| Same grounded-vs-fallback decision (`hasGroundedInsight`, `resolveCoachGeneration`) | Generated, `SOURCES`-listed — identical by construction |
| Same generated validation function (`combinedValidate`) | Generated, `SOURCES`-listed as of this fix — identical by construction |
| Same rejection consequence semantics (`applyRejectionConsequences`, `buildFailureSentinelRow`) | Generated, `SOURCES`-listed — identical by construction |
| Same `habit_id` persistence rule (`selectedHabit ? selectedHabit.habitId : null`) and same sentinel-TTL interpretation (`isSentinel ? isWithinFailureBackoff(...) : ...`) | **Not generated** — a small, legitimately-restated (different variable names, different call shapes) piece of hand-maintained logic in each file. Pinned by `scripts/edge-function-wiring.test.ts`, which extracts and executes both files' real source (`generateInsight` from `ai-insights/index.ts`, `processRecipient` from `send-coaching-push/index.ts`) against the same fixtures and cross-checks their outputs directly — not a second byte-identity test, and not a reimplementation of either function. |

`scripts/edge-function-wiring.test.ts` is therefore the D1 mechanism for the one class of shared behaviour that cannot be made generated-identical (it depends on each function's own local variables and call shape) but must not be allowed to silently diverge in meaning.

### Sentinel stamping for a pre-existing rejection, a Step 5 finding not named in the earlier ruling **[New]**

§6.6's ruling that a rejection "marks the day's push attempt handled" was written for the case where *this* cron tick causes the rejection. Wiring `processRecipient` surfaced an adjacent case the ruling didn't name: a cron tick that finds an **already-existing** fresh sentinel (written earlier that day by a previous tick, or by the interactive `ai-insights` path) has caused no rejection of its own, so `applyRejectionConsequences`'s `markCronPushHandledToday` never runs for it. Left unstamped, every subsequent tick that day would re-read the same cached sentinel and skip again, indefinitely, without ever recording that the day was handled. `processRecipient` now also stamps `coach_push_last_sent_date` when it finds a pre-existing fresh sentinel (`content === ''` and still within `isWithinFailureBackoff`), not only when it produces one itself.

### `processRecipient` / `generateInsight`: a deliberate testability refactor **[New]**

`send-coaching-push`'s old `generateNudge` plus its inline per-recipient loop body, and `ai-insights`'s inline `Deno.serve` logic, were both extracted into named functions (`processRecipient(supabase, anthropic, recipient, today)`, `generateInsight(supabase, anthropic, userId, kind, config, today)`) returning plain data (a boolean; a `{status, body}` pair) rather than a `Response`. This is larger than the minimum wiring would have required, done deliberately so `scripts/edge-function-wiring.test.ts` can extract and execute the *real* production source directly against mocked `supabase`/`anthropic`, rather than reimplementing the wiring to test it — the same rationale already applied to `scripts/edge-function-mappers.test.ts` for the row mappers, extended here to the orchestration call sites themselves.

### `consistencyPct` omission: confirmed by inspection, not a new fixture **[New]**

The wiring preserves `buildHabitCoachFacts`'s existing omit-on-`null` behaviour for `consistencyPct`/`consistencyWindowOpportunities` (Step 2, already tested in `coach-facts.test.ts`). This was **not** given a new dedicated end-to-end fixture test in Part 3b: constructing a habit that is simultaneously grounded (a qualifying `momentumState` or positive Habit Health) *and* has zero Scheduled Opportunities in its own consistency window would require an artificial mid-history pause fixture (paused for the entire window while an earlier stretch still supports a qualifying Momentum State), materially more complex than the property being checked warrants. Confirmed instead by direct inspection: the old `Math.round(consistencyRate * 100)` inline construction is gone entirely from both call sites, replaced by passing `buildCoachFacts`'s own output straight through with no post-processing of any kind — there is no `?? 0` or equivalent default anywhere in the new code, and nothing in the diff could reintroduce one. Recorded here rather than manufactured into a fragile test.

### Fresh grounded-rejection response now carries `habitId`, matching the cached read **[New — corrects an inconsistency]**

The grounded rejection path always persisted the selected habit's id on the failure sentinel, but the immediate JSON response it returned omitted `habitId` entirely, while a later cache-hit read of that same sentinel row correctly returned it (`existing.habit_id`). Fixed: the fresh rejection response now returns `habitId: rejectedHabitId ?? undefined`, using the same selected-habit value already being persisted, so the fresh and later-cached responses for the same rejection are shape-consistent. Pinned directly in `scripts/edge-function-wiring.test.ts` (`generateInsight`'s habit_id/response-shape suite), including a same-sentinel fresh-vs-cached comparison.
...
### Cross-kind fallback collision, amended, deployed and browser-verified **[Ruled — amends the seed-resolution ruling above]**

First live acceptance check (2026-09-14) showed Coach (nudge) and Weekly reflection displaying identical fallback text. Confirmed cause: `buildFallbackProvider`'s seed was `userId + dayKey` only, deliberately ignoring `kind` (`coach-fallback.ts`'s own doc comment; `coach-fallback.test.ts` pinned this as intended). Would also have affected `monthly` on the same day.

**The original seed-resolution ruling remains historically valid for the case it considered.** It reasoned about one account-level fallback message per coaching request and correctly resolved the seed to `userId + dayKey` for that case. It did not contemplate multiple insight kinds being displayed together on one screen — that scenario did not exist until Step 5 wired the fallback into both Edge Functions' ordinary generation paths. This is not a defect in the original reasoning; it is a case the reasoning was never asked to cover.

**First attempt, superseded before deployment.** Mixing `kind` directly into the hashed seed (`userId|dayKey|kind`) was implemented first, but proven insufficient by a direct search against that implementation before it shipped: 52% of 30,000 sampled account/day pairs produced a cross-kind collision — matching the rate three independent draws into five buckets would produce. Hashing `kind` in only re-randomises the outcome per kind; it does not guarantee the three are different. A test asserting one fixture (`user-42`/`2026-09-12`) happened to produce three distinct messages was not, and could not have been, proof of the general case.

**Final ruling, 2026-09-17.** The base account/day hash is unchanged from the original ruling — `userId + dayKey`, still the sole variation mechanism, still exactly the original formula. `kind` is applied afterward as a fixed, unique additive offset (`KIND_OFFSETS`, typed `Record<CoachFactsKind, number>` so a future fourth `CoachFactsKind` value forces this map to be revisited at compile time): `nudge: 0`, `weekly: 1`, `monthly: 2`. With `FALLBACK_MESSAGES.length` (5) at least the number of `CoachFactsKind` values (3) and each offset distinct modulo that length, the three kinds' final indices are **pairwise distinct for every possible base, unconditionally** — a proof (verified exhaustively over all 5 possible base values, and confirmed by a 30,000-pair sweep finding zero collisions), not a statistical property. `nudge`'s offset of 0 makes its formula byte-identical to the original pre-amendment ruling, confirmed by running the original table's exact seed values against the corrected implementation, not merely by argument.

**The fixed cyclic relationship between kinds is consciously accepted.** Two or three kinds shown together will always occupy `message[N]`, `message[N+1 mod 5]`, `message[N+2 mod 5]` for whatever `N` that account/day produces — a fixed relative structure, though the actual messages shown still vary account-to-account and day-to-day. This is accepted rather than closed with a second permutation layer: message order is never shown to a user (the message set's own doc comment already states order carries no ranking meaning), so there is no realistic path for this relationship to be perceived, and a second layer would add real complexity — another hash, more test surface, the same class of bug just found — for no observable benefit. The message set, FNV-1a, and the deterministic (non-random) selection design are all unchanged.

**Deployment consequence, unchanged from the original finding.** `coach-fallback.ts` is `SOURCES`-listed, so this moves the shared `block` hash and requires both Edge Functions regenerated, re-pasted, and B6-reverified; not eligible for the §2/§6.8 single-function exception, since it changes cache-content-producing logic.

**Deployed and browser-verified, 2026-09-17.** Both Edge Functions pasted from commit `557b027` and B6-verified live: shared `block` `91808f504d92`, `ai-insights` file `1000424b591c`, `send-coaching-push` file `038b4436b4ea`, MATCH on both. The CORS fix (§6.8) was re-checked at the same time, not assumed still correct: the allowed-origin OPTIONS probe (`http://localhost:8081`) returned exactly that `Access-Control-Allow-Origin`, and an unrelated-origin probe correctly returned none.

A single controlled Progress load then verified the fallback design itself, not merely that two sentences differed. Before the load, cache state was deliberately prepared as a narrow test-preparation action, distinct from the earlier Part 3c cutover clear: the existing `nudge` row had already expired naturally, and the existing `weekly_reflection` row (still fresh, and carrying the pre-fix collided text) was removed by a targeted delete matching only `user_id` + `kind: 'weekly_reflection'` + its exact `created_at`. The real deployed `selectFallbackIndex`/`KIND_OFFSETS` were then run directly (not reimplemented) for day key `2026-09-17` and this account, predicting `nudge` index 4 ("Putting a visible reminder...") and `weekly` index 0 ("Pick the smallest possible first step..."), **before** the load. The Coach and Weekly reflection cards showed exactly that predicted pair on their correct surfaces on the one load performed. This is stronger evidence than observing two different sentences: the specific pair was named in advance from the deployed logic and matched exactly, including correctly anticipating that today's `weekly` text would coincidentally match the old buggy row's text for an unrelated reason (index 0 for this day/account), which was identified before the load so it could not be mistaken for the fix not working.

**Legacy-symbol `SOURCES` cleanup (§6.3) shipped in the same paste and is deployed and verified by the same B6 match** — both changes landed in one generated block, and the live `block`/`file` hashes matching the regenerated committed source confirms both simultaneously; no separate deployment step exists for it.
...
**Live closeout, 2026-09-15/17.** `ALLOWED_ORIGIN_DEV` set to `http://localhost:8081`; `ai-insights` pasted from `266068e`; B6 verified live (`block f69ccd51812b`, `file 0c0ad2d65daf`, zero-habit canned response). Both OPTIONS probes verified: matching origin echoed exactly, unrelated origin received no `Access-Control-Allow-Origin` header, both 200, `Access-Control-Allow-Headers`/`Vary` unchanged in both. Controlled browser acceptance: one Progress load produced two `ai-insights` fetch entries in Safari from `Origin: http://localhost:8081`. The same load wrote two fresh `ai_insights` rows for the test account: `nudge` at `2026-09-14 23:24:20.28459+00` and `weekly_reflection` at `2026-09-14 23:24:20.306056+00`; neither was a sentinel and both had `habit_id = NULL`, consistent with account-level fallback. The historical Supabase Invocation rows were no longer available when checked two days later, so HTTP method/status were not independently retained. Together with the successful allowed/unrelated-origin OPTIONS probes and populated UI, this closes the CORS acceptance point.
...
**Applied, 2026-09-20.** `docs/implementation-roadmap.md:66` now reads the corrected wording above.
...
| **A second, distinct `extractDeclarations` truncation class, found and fixed during Step 5 Part 3b: a multi-line `type` union with more than one independent `\| {...}` member truncated after the first member. [Fixed]** `CoachGenerationResult` (`type X = \| {...} \| {...} \| {...};`, the middle transition separated by an interior doc comment) was the first `SOURCES` entry ever to have this shape. The extractor's depth tracking treats "brace depth returned to 0 after opening" as "declaration complete" — true for a single-brace type (the whole existing whitelist, until this one), false for a multi-line union where each member closes its own brace back to depth 0 independently. Caught simultaneously and independently by both `scripts/build-edge-functions.ast-equivalence.test.ts` and the Step 1 strict type-check guard (a real, cascading `tsc` error two declarations later, from the truncated type silently absorbing the next declaration's leading text) — the same multi-layered defence that caught the original `meetsRateWindow` truncation, working as designed a second time, on a different trigger. | **Fixed in `extractDeclarations`:** a `type` declaration now scans until a semicolon at brace/paren depth 0, rather than stopping the instant depth first returns to 0. `function`/`const` bodies are unaffected (their own closing brace is genuinely their end). Verified against every existing `SOURCES` symbol via the AST-equivalence test, not only the new one. |
| **`coach-orchestration.ts`'s `resolveCoachGeneration` had one un-narrowed literal (`{ path: 'fallback', ... }`, missing the `as const` its two sibling branches already had), invisible until this symbol was actually spliced into `SOURCES` for the first time in Step 5 Part 3b. [Fixed]** Passed every check while `coach-orchestration.ts` was unwired (Part 1's own Jest suite, and a whole-repository `tsc --noEmit`), because both tolerate a wider inferred type in that position. The Step 1 strict guard's isolated, whole-file check of the actual spliced text does not, and failed loudly the moment this file entered `SOURCES`. | **Fixed:** added the missing `as const`, a pure type-narrowing annotation with no behavioural change — confirmed by the guard passing afterward with no other change. Recorded because it shows the guard catching something neither the domain test suite nor a full-project `tsc` run ever would, precisely because splicing into an isolated file is a materially different consumption context. |
...
| **B6 verification proves deployed-source identity and no-Origin reachability, never browser-CORS reachability. [New]** Every B6 check in this phase used `curl`, which performs no preflight and enforces no browser CORS policy — so a stamp match, correct `file`/`block` fingerprints, and a successful no-Origin `curl` POST could all pass cleanly while the web app remained unreachable from a real browser due to an `Access-Control-Allow-Origin` mismatch (§6.8). | Accepted as a structural verification gap. **Practical implication:** any browser-consumed Edge Function with CORS behavior needs an explicit browser-origin/OPTIONS check alongside the existing B6 stamp check; this does not change what the `block`/`file` fingerprints themselves mean. |
| **Cross-kind fallback collision, confirmed 2026-09-14, resolved and deployed 2026-09-17 (§6.5, "Cross-kind fallback collision, amended, deployed and browser-verified").** Coach and Weekly reflection could show byte-identical fallback text on the same screen, since `buildFallbackProvider`'s seed ignored `kind`. A first fix (hashing `kind` into the seed) was proven insufficient — 52% collision rate over 30,000 sampled pairs — before it shipped. | **Resolved, deployed, and browser-verified.** Base hash unchanged (`userId + dayKey`, the original formula); `kind` now applies a fixed unique offset (`KIND_OFFSETS`), guaranteeing pairwise-distinct selection for every account/day, proven exhaustively. Live on both functions (`block 91808f504d92`), B6-matched, and confirmed by a predictive browser check (exact predicted pair appeared on the correct surfaces). |
| **Grounded coaching live-generation sample, obtained 2026-09-20.** Prior to this date, every observed live generation had taken the deterministic fallback path, leaving the grounded prompt, numeric validator, lexical backstop, prompt constraint rules (rule 7 named specifically, given its absent mechanical backstop), and the validator/rejection path all unexercised against genuine model output. | **Partially resolved — scope stated precisely, not closed universally.** One controlled sample obtained and accepted: dedicated test account, `nudge`, momentum `building`, one habit, sparse facts (two recovery fields omitted), accepted (non-sentinel) output, checklist fixed in advance and passed in full. **Not exercised by this sample:** grounded `weekly`/`monthly`, the other three qualifying momentum states (`thriving`/`recovering`/`rebuilding`), `positive_recent_comparison`, a stated lapse reason, or a live validator rejection. See `docs/acceptance-gate-findings.md` for the classification of which of these are covered by existing automated tests versus left as open residual notes — not every combination is automatically a further live-test requirement. |
...
| **The `.env` publishable-key path is untested. [New]** `.env` carries an `sb_publishable_...` value under `EXPO_PUBLIC_SUPABASE_ANON_KEY` (recorded in `CLAUDE.md`'s "Deployment stamp" section, as of 2026-09-10). Every B6 verification performed in this phase used the legacy `eyJ...` anon key instead, not the publishable key. The publishable key is untested against these Edge Functions — **not known to fail, only unverified.** The browser client (`lib/supabase.ts`) authenticates using whatever value `.env` actually supplies, which is the publishable key, not the legacy key B6 verified against. No inference is drawn about what would happen if the legacy key were deprecated or rotated; only that in that event, the currently-untested publishable-key path would become the one actually load-bearing for every browser request, with no live verification behind it today. | Accepted, recorded rather than tested. Revisit before any change that removes or rotates the legacy `eyJ...` key, or when the publishable key can be verified without disturbing the current known-good B6 baseline. |
...
| Location | Claim that becomes false | Breaks at | Status |
|---|---|---|---|
| `CLAUDE.md:146` — "Phase 5 constraint" | the block includes only `day-key.ts` and `habit-stats.ts`; neither function can access `momentum.ts` or `recovery.ts` | Step 3 | **Resolved 2026-09-20** — rewritten as a historical note |
| `CLAUDE.md:85` | "regenerates a whitelisted subset of `day-key.ts` + `habit-stats.ts`" | Step 3 | **Resolved 2026-09-20** — now points to the script's own `SOURCES` list as authoritative rather than restating it |
| `CLAUDE.md:30` — copy-variation contract rule 4 | same whitelist claim, as a stated Phase 8 prerequisite | Step 3 | **Resolved 2026-09-20** — Phase 8 prerequisite re-justified against the real remaining gap (`lib/notifications.ts` has no state-as-data wiring), not the now-false Edge Function claim |
| `docs/implementation-roadmap.md:161` — Phase 8 | same claim, as a stated prerequisite | Step 3 | **Resolved 2026-09-20**, same correction |
| `CLAUDE.md`, AI coaching section | `calendar*` usage, the 7/14/30-day windows, absence of output validation, the rate-limit description | Steps 4-5 | **Resolved 2026-09-20** — rewritten to describe the no-fixed-lower-bound read, `buildCoachFacts`/Route C/`combinedValidate` pipeline |
| `lib/domain/habit-stats.ts` doc comments on both `calendar*` functions | "neither Edge Function fetches `habit_schedule_periods` today" | Step 4 | **Resolved 2026-09-20** — also updated to record their Step 5 Part 3 removal from `SOURCES` |
| `scripts/build-edge-functions.js` header comment | same claim, plus the `SOURCES` rationale paragraph | Steps 3-4 | **Already resolved** — rewritten during the Step 5 Part 3 legacy-symbol cleanup |

**All rows resolved, 2026-09-20. Documentation reconciliation is complete.**
...
**Status, 2026-09-20 (updated).** All eight completion criteria satisfied: **item 7** (Acceptance Gate wording, §9) applied; **item 8** (documentation reconciliation table above) complete. The formal Acceptance Gate document (`docs/acceptance-gate-findings.md`) has been created — see below. **Resolved and deployed:** the §6.3 legacy-symbol cleanup check and the §6.5 cross-kind fallback amendment, both shipped in the same both-functions paste (`block 91808f504d92`), B6-matched live on both functions, the fallback amendment additionally confirmed by a predictive browser check. Diagnostic removal is **not** on this list — confirmed a Phase 6 precondition only (see "One obligation outlives Phase 5" above).

**Grounded coaching, live evidence obtained 2026-09-20 (§10, "Grounded coaching live-generation sample") — scope stated precisely, not closed universally.** A genuine live grounded generation has now been demonstrated end-to-end on one controlled, representative case (a dedicated test account, `nudge`, momentum `building`, one selected habit, a sparse facts payload with two omitted recovery fields), verified against a checklist fixed in advance from the deployed prompt/validator/lexical rules, and accepted. The full evidence scope — what this sample does and does not cover — is recorded in `docs/acceptance-gate-findings.md` and must not be read as covering every kind/state combination; see that document for which remaining combinations are treated as closed by existing automated coverage versus left as open residual notes.
