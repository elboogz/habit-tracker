...
**Logged 2026-08-20, corrected and resolved 2026-08-22.** The original finding overstated the gap: on Edit Habit, swipe-down-to-dismiss already worked, and the date correction panel's count-based branch already had a working no-op close ("Done"). The actual defects were narrower:

- **Edit Habit had no visible exit control on either platform** — not a missing gesture, a missing affordance. `app/_layout.tsx` set `headerBackTitle: 'Cancel'` on the route, but `presentation: 'modal'` makes the screen the root of its own presented stack, so native-stack never generates the automatic back chevron that label would have attached to (a react-native-screens/UIKit modal-presentation limitation, confirmed by tracing `useHeaderConfigProps.js`: `hideBackButton`/`backTitle` are computed correctly and forwarded to the native header, but nothing on a modal root synthesizes a back element to receive them). The header rendered — the title "Habit" showed — just with no back button in it. Android has no swipe-down convention at all, so this was a genuine dead end there, not just an iOS discoverability issue. Fixed by giving `app/habit-form.tsx` its own `<Stack.Screen options={{ headerLeft: ... }}>` rendering an explicit `HeaderBackButton` (`@react-navigation/elements`, already a dependency) labeled "Cancel" that calls `router.back()`; the now-inert `headerBackTitle: 'Cancel'` was removed from `app/_layout.tsx`. Discards silently, no confirmation — consistent with this app's existing convention of confirming only genuinely destructive writes (delete habit, zeroing a completion), never abandonment of never-committed form state. Applies identically to habit creation, since it's the same route and header configuration.
- **The date correction panel's simple-habit branch had no equivalent to the count-based branch's "Done" button** — only the Mark done/Mark not done toggle, which always wrote a change. Fixed by adding a matching "Done" close control to that branch that calls `onClose` alone. Also fixed a related bug surfaced while tracing this: confirming the "Remove this completion?" dialog for a simple habit left the panel open, because the confirm callback never called `onClose`. `requestSetAmount` now takes an optional `onDone` callback, invoked once the write is actually applied (immediately, or after the user confirms) — every path out of the panel now closes it consistently.

Both fixes implemented and verified (`npx tsc --noEmit`, `npm run lint`, `npm test` all clean; no component tests exist yet for either screen per CLAUDE.md, so this was also traced manually through the code paths).
...
- **Recovery Card entry-point discoverability** (recorded during the Phase 5 Step 4 soak check, 2026-09-11). The Today screen's Recovery Card header — "N habit(s) are ready for a fresh start / Tap to see your options" — did not read as an interactive entry point on first encounter: it presents as a dismissible notification banner (an × affordance on the right, sitting above the habit list, `components/recovery-card.tsx`), and the options were looked for on Progress and on individual Habit Detail screens before the banner was recognised as the way in. Weighed above a typical cosmetic nit because returning after a lapse is the product's core premise, and this banner is the surface where a returning user meets it; it is also the single most dismissible-looking element on the screen it appears on. **Not a Phase 5 defect** — the eligibility logic and the tap-to-expand interaction both work exactly as built (`app/(tabs)/index.tsx`'s eligibility predicate, `RecoveryCard`'s `expanded` state) — nothing is broken, and nothing was changed here. Deferred to Phase 6 / product polish; not redesigned as part of this recording.
