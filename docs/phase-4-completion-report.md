...
**Status: paused.** Work on the confirmation mechanism stops here pending a product decision on admissibility. Phase 5 remains not started.
...
**Not resolved here, and not to be resolved by implementation or a new behavioural rule.** Recorded as an open product decision blocking Phase 5 (see below), independent of the confirmation-mechanism blocker. Noted for whenever a decision is made: one option under consideration is a condition-aware narrative sentence on the existing `insufficient_data` state rather than a new `MomentumStateKey` value — sharing a presentation mechanism with Item 2's already-deferred narrative override. The confirmation-mechanism work above is shaped so it doesn't foreclose that route: the parked floor rule (below) only ever affects which *state* confirms, never how a *state's copy* is derived, so a future narrative-level resolution for this item remains available regardless of how the floor's admissibility is ultimately decided.

### Open product decisions blocking Phase 5

Two decisions are outstanding. Neither is resolved by implementation or by introducing a new behavioural rule in this pass:

1. **Admissibility of the positive-family evidence floor.** Whether treating `recovering`/`rebuilding` as affirmative evidence capped at the chain's weakest rung (`building`), for `computeConfirmedMomentumState`'s Rule 2 only, is consistent with "do not propose an ordering between off-chain states and the chain," or whether it crosses that line despite never comparing off-chain states to each other or exposing a general rank. Blocks resolution of the three-history monotonicity violation, which in turn blocks shipping the otherwise-settled current-day semantics fix.
2. **Treatment of sustained/moderate alternating engagement currently trapped at `insufficient_data`.** Whether this needs a new `MomentumStateKey`, a condition-aware narrative override on an existing state, or some other resolution — and, if the latter, how it relates to Item 2's already-deferred narrative override. Blocks Phase 5 independently of decision 1, since the AI Coach would otherwise narrate "still gathering enough days to show a pattern" against a history with a demonstrable, high-confidence recovery track record.

Work on the confirmation mechanism is paused pending resolution of decision 1. Phase 5 has not begun.
